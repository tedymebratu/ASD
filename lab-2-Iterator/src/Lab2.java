...
public class Lab2 extends JFrame
{
	...
	void JButtonAllSwimmers_actionPerformed(ActionEvent event) throws Exception {
//		Vector vectorlist = slist.getVector();
		IteratorA iterator = slist.createIterator();

		while (!iterator.isDone()){
			Swimmer swimmer = (Swimmer) iterator.currentItem();
...
//		}
	...
	void JButtonAllBackward_actionPerformed(ActionEvent event)throws Exception
	{
		//Vector vectorlist = slist.getVector();
		IteratorA iterator=slist.createReverseIterator();

        /*for(iterator.current();!iterator.isDone();iterator.next()){
            Swimmer swimmer = (Swimmer) iterator.currentItem();
            JTextArea1.append(swimmer.getFname()+" "+swimmer.getLname()+ " \n");
		...
		/*for(int i=iterator.size();i>=0;i--){
			iterator.setCurrent(i);
            Swimmer swimmer = null;
            try {
                swimmer = (Swimmer) iterator.currentItem();
            } catch (Exception e) {
                e.printStackTrace();
            }
            JTextArea2.append(swimmer.getFname()+" "+swimmer.getLname()+ " \n");
